...
<body class="w-full h-full bg-gray-100">
        ...
        <div class="text-center pt-20">
            ...
            <div class="justify-center-end">
                <form action="{{ route('logout') }}" method="POST">
                    @csrf
                    <button type="submit"
                        class="primary-btn inline text-base sm:text-xl bg-red-500 py-4 px-4 shadow-xl rounded-full transition-all hover:bg-red-400">
                        Logout
                    </button>
                </form>
    ...
    @if (session()->has('message'))
        <div class="w-4/5 mx-auto mt-10 pb-10">
                ...
                <div class="w-11/12 mx-auto pb-10">
                    ...
                    @if (Auth::id() === $post->user->id)
                        <a href="{{ route('blog.edit', $post->id) }}"
                            class="block italic text-green-500 border-b-1 boder-green-400">Edit</a>
                        <form action="{{ route('blog.destroy', $post->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
                                class="block italic text-red-500 border-b-1 boder-red-400">Delete</button>
